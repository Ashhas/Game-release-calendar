  ...
  group('UpcomingGamesState', () {
    ...
    test('constructor with games data sets it correctly', () {
      final testSections = [
        GameSection(
          ...
          games: [_createTestGame(1)],
        ),
        GameSection(
          date: DateTime(2025, 1, 2),
          precision: DatePrecision.exactDay,
          games: [_createTestGame(2), _createTestGame(3)],
        ),
      ];
      final gamesAsyncValue = AsyncValue.data(testSections);

      final state = UpcomingGamesState(games: gamesAsyncValue);

      expect(state.games, equals(gamesAsyncValue));
      state.games.whenData((sections) {
        expect(sections.length, equals(2));
        expect(sections.first.games.length, equals(1));
        expect(sections[1].games.length, equals(2));
      });
    });
    ...
    group('copyWith', () {
      test('creates new instance with updated games', () {
        ...
        final newGamesValue = AsyncValue.data(testSections);
      ...
      test('preserves existing values when parameters are null', () {
        ...
        final initialState = UpcomingGamesState(
          games: AsyncValue.data(testSections),
          selectedFilters: customFilter,
      ...
      test('creates new instance with multiple updated properties', () {
        ...
        final updatedState = initialState.copyWith(
          games: AsyncValue.data(newSections),
          selectedFilters: newFilter,
          nameQuery: newQuery,
        );

        expect(updatedState.games.value, equals(newSections));
        expect(updatedState.selectedFilters, equals(newFilter));
        expect(updatedState.nameQuery, equals(newQuery));
      });
    });
    ...
    group('AsyncValue games handling', () {
      test('handles loading state', () {
        const loadingState = AsyncValue<List<GameSection>>.loading();
      ...
      test('handles error state', () {
        final errorState = AsyncValue<List<GameSection>>.error(
          'Network error',
          StackTrace.current,
        );
      ...
      test('handles data state with empty sections', () {
        const dataState = AsyncValue<List<GameSection>>.data([]);
